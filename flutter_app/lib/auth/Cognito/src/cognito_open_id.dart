...
class CognitoOpenIdCredentials {
  String _region;
  String _userPoolId;
  ...
  CognitoOpenIdCredentials(String identityPoolId, CognitoUserPool pool,
  ...
  Future<void> getAwsCredentials(token, authenticatorName) async {
    if (expireTime == null ||
        new DateTime.now().millisecondsSinceEpoch > expireTime - 60000) {
      final identityId = new CognitoIdentityId(_identityPoolId, _pool);
      final identityIdId = await identityId.getIdentityId(
          token, authenticatorName);

      if (authenticatorName == null) {
        authenticatorName = 'cognito-idp.$_region.amazonaws.com/$_userPoolId';
      }
      final authenticator = authenticatorName;
      ...
      var data;
      try {
        data = await _client.request('GetOpenIdToken', paramsReq,
        ...
        if (e.code == 'NotAuthorizedException' && _retryCount < 1) {
          _retryCount++;
          return await getAwsCredentials(token, authenticatorName);
        }
